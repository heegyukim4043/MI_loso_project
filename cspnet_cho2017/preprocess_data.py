...
SAVE_DIR = os.path.join(os.path.dirname(__file__), "..", "preprocessed")
...
def preprocess_dataset(dataset, subjects, sfreq_target, save_path,
    ...
    for subj in subjects:
        ...
        try:
            # Use _get_single_subject_data directly to bypass MOABB 1.5.0 bug
            # where get_data filters session keys "0","1" against selected_sessions
            # (1,2), silently dropping session "0" (the first recording day).
            # Then manually apply SetRawAnnotations (normally done by get_data's
            # process pipeline) so each run has the correct event annotations.
            from moabb.datasets.base import SetRawAnnotations
            _annotate = SetRawAnnotations(dataset.event_id, dataset.interval)
            sessions_raw = dataset._get_single_subject_data(subj)
            sessions = {
                sk: {rk: _annotate.transform(rv.copy())
                     for rk, rv in sd.items()}
                for sk, sd in sessions_raw.items()
            }
